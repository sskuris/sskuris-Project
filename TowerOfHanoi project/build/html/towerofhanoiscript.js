...
function start_hanoi(disk_count, start_bar, target_bar, support_bar) { 
  ...
  if(disk_count == 1){ 
      ...
      get_bar_array();
      // memory_list.push([start_bar, support_count, target_count]);
      get_disk_move(start_bar, target_bar);
      // console.log(start_bar, support_bar, target_bar);
      console.log(total_arr)
      // memory_list.push(total_arr);
      let second_array = []
      for(let i=0; i<total_arr.length; i++){
        let third_array = [];
        for(let j=0; j<total_arr[i].length; j++){
...
let output_text = () => {
  for(let i=0; i<text_input.length; i++) {
    setTimeout(() => {
      $("#inner_box").append(`<p class="append_text" id="append_text">${text_input[i][0]}번째 : ${text_input[i][1]}번 bar에서 ${text_input[i][2]}번 bar로 이동 </p>`);
